...
module top (
    ...
    localparam [7:0] P_FM_STRIKE= 8'h12;   // write any value -> FM note trigger
    localparam [7:0] P_FM_AMP   = 8'h13;   // FM strike amplitude
    localparam [7:0] P_FM_WIDTH = 8'h14;   // FM brightness / index
    localparam [7:0] P_FM_DAMP  = 8'h15;   // FM decay loss
    ...
    initial begin
        ...
        params[P_FM_AMP]    = 8'd180;
        params[P_FM_WIDTH]  = 8'd24;
        params[P_FM_DAMP]   = 8'd2;
    ...
    always @(posedge CLK) begin
        fm_strike <= uapplied && (uparam == P_FM_STRIKE);
    ...
    always @(posedge CLK) begin
        ...
        if (slot_dd == NM1) begin
            ...
            acc    <= 28'sd0;
            sample <= FM_ENABLE ? fm_sample : ((acc + mixsample) >>> 11);
        end else begin
            acc <= acc + mixsample;
        end
    end
    ...
    reg signed [15:0] fm_frame_sample = 16'sd0;

    fm_voice fm_inst (
        .clk(CLK),
        .audio_tick(i2s == 6'd0),
        .pluck(fm_strike),
        .carrier_tw(32'd5039428),
        .strike_amp({1'b0, params[P_FM_AMP], 7'd0}),
        .strike_width(params[P_FM_WIDTH]),
        .damping(params[P_FM_DAMP]),
        .sample(fm_sample)
    );
    ...
    always @(posedge CLK)
        if (i2s == 6'd0)
            fm_frame_sample <= fm_sample;

    wire signed [15:0] serial_sample;
    assign serial_sample = FM_ENABLE ? fm_frame_sample : sample;
    assign dout = serial_sample[16 - i2s[4:1]];
